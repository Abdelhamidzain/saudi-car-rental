...
export async function generateMetadata({ params }: { params: Promise<{ city: string; category: string; car: string }> }): Promise<Metadata> {
  ...
  // Augment (not replace) static city/category/car identity with DB overlay.
  // Pricing stays static — Product JSON-LD lowPrice/highPrice and `car.dailyPrice`
  // interpolation are unaffected by this adapter (deferred to a future pricing task).
...
export default async function CarPage({ params }: { params: Promise<{ city: string; category: string; car: string }> }) {
  ...
  // Augment (not replace) static city/category/car identity with DB overlay.
  // Static rows still passed to generateCarSEOContent (content blocks stable)
  // and pricing (car.dailyPrice/monthlyPrice, Product JSON-LD) stays on static.
  const overlay = await getCarPageOverlayFromDb(cs, cats, cars)
  const cityNameAr = overlay?.cityNameAr ?? city.nameAr
      ...
      <section className="hero">
        ...
        <div className="container">
          <div className="hero-inner">
            ...
            <div id="form"><LazyLeadForm defaultCitySlug={city.slug} selectedCarSlug={car.slug} defaultCategorySlug={cat.slug} /></div>
          </div>
        </div>
      </section>
