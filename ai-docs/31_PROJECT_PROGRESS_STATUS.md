...
**Commit:** `c0b1bc4` (current `main` HEAD).
- **Files changed (3):**
...
Phase 6 is now structurally complete: all four public detail routes are on the augmentation pattern (`/sa/airports/[airport]` 6.2A, `/sa/[city]` 6.2B, `/sa/[city]/[category]` 6.2C, `/sa/[city]/[category]/[car]` 6.2D), the layout-level Riyadh `LocalBusiness` leak is fixed (6.2X), and an automated regression sweep covers all the above (6.3 — `npm run seo:check`, 240/240 PASS). Pricing is still static everywhere; `/` and the trust pages have not been migrated. Two candidate next tasks have surfaced.

### Candidate A — Task 7: Production launch readiness (Vercel) — **RECOMMENDED**

The repo has the migration, the regression sweep, and the admin dashboard all working locally. The remaining gap to a launchable site is **infrastructure verification on a real Vercel environment**: env-var configuration, secret hygiene, route caching, robots/sitemap accessibility, and end-to-end smoke tests against the prod URL rather than `localhost:3299`.

**Why recommended as the safest next step:**
- All code paths have been smoke-tested locally; the cheapest remaining risk is environment misconfiguration on Vercel (wrong env var, missing service-role key, cache header drift).
- No app file changes are expected — this is config + verification.
- The work is reversible by toggling Vercel env vars; no DB or schema impact.
- Pricing migration (Candidate B) is a content-quality improvement; production-readiness is a launch blocker. Land the launch blocker first.

**Recommended scope (to be detailed in a plan after approval):**
1. Verify Vercel project env vars match the local `.env.local` set:
   - `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY` — exposed to the browser; safe.
   - `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_PROJECT_REF` — server-only; must not leak.
2. Run `npm run seo:check` against the deployed Vercel preview URL (the script's `BASE` could be made env-configurable in a small follow-up).
3. Run `npm run seo:check -- --fallback` once against the deployed URL to verify the DB-overlay fallback path works in the Vercel runtime.
4. Confirm `/admin/*` returns redirect-to-login for unauthenticated visitors on the deployed URL.
5. Verify `robots.txt` and `sitemap.xml` are reachable; verify response caching on prerendered routes.
6. Run the manual checks from [ai-docs/32_PRE_LAUNCH_REGRESSION_CHECKLIST.md](32_PRE_LAUNCH_REGRESSION_CHECKLIST.md) §3 against the prod URL: Lighthouse, schema.org validator, mobile visual, accessibility.
7. Document the launch sign-off result.

**Expected file budget:** 0–1 app files (no changes expected); 1 docs update (the `31_*` status doc); potentially 1 small enhancement to `scripts/seo-regression.ts` to accept a `BASE=https://...` env var so it can target the deployed URL.

### Candidate B — Task 6.4: Public Pricing Strategy / Offers Price Overlay Planning
